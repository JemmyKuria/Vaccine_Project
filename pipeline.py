...
# paths
MODEL_PATH = "multi_tuned_rf.pkl"   # we'll load the MultiOutput model you saved

# load once
model = joblib.load(MODEL_PATH)
print("Model loaded:", model)

# hard-code the column order you trained with
EXPECTED_COLS = [
...
]   # <-- copy the exact list from your training notebook


# ------------------------------------------------------------------
# 1. Pre-processing (same steps you used in the notebook)
# ------------------------------------------------------------------
def preprocess(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Takes raw uploaded file and returns X ready for model.predict().
    ...
    # ---- handle high-missing employment columns (in case user uploads them)
    ...
    # ---- one-hot nominal columns (same list you used)
